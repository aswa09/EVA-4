            ...
            if (
                ...
                if mode == 'iteration':
                    pbar.set_description(
                        desc=f'lr={self.history["lr"][-1]} Iteration={iterations - 1} {self.metric}={metric_value:0.2f}')
                ...
                if mode == 'epoch':
                    ...
                    pbar.set_description(
                        desc=f'lr={self.history["lr"][-1]} Iteration={iteration} {self.metric}={metric_value:0.2f}')

        metric = self._display_metric_value(self.best_metric)
        if mode == 'epoch':
            print(f'Learning Rate: {self.best_lr:.4f}, {self.metric.title()}: {metric:.2f}\n')
        # elif mode == 'iteration':
        #     print(f'lr={self.best_lr} {self.metric.title()}={metric}')
