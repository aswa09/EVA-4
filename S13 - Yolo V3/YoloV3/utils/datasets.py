...
import glob
import os
mydir = "C:/Users/Aswathi/Desktop/EVA4B2/s13/YoloV3/data/customdata/images/"
sub_dir=[x[0] for x in os.walk(mydir)]
sub_dir=sub_dir[1:]
print(len(sub_dir))

file_list=[]
for sd in sub_dir:
    file_list+=glob.glob(sd + "/*.*")
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=416):
        ...
        if os.path.isdir(path):
            ...
            if len(files)==0:
                sub_dir = [x[0] for x in os.walk(path)][1:]
                # print(sub_dir)
                for sd in sub_dir:
                    files += glob.glob(sd + "/*.*")
                    files=sorted(files)
